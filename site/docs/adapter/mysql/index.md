...
> Here you’ll learn about the particularities of the [MySQL][2] adapter.
...
Pass the `mysql.ConnectionURL` value as argument to `mysql.Open()` so the
session is created.
...
> The `mysql.ParseURL()` function is also provided in case you need to convert
> the DSN into a `mysql.ConnectionURL`:
